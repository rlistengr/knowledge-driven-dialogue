...
def prepare_batch_data(insts,
    ...
    batch_indexs = [inst[5] for inst in insts]
    batch_weight = [inst[6] for inst in insts]
    batch_oneforlog = [inst[7] for inst in insts]
    ...
    for i in range(len(insts)): 
        if 'kn' in task_name: 
            out = [context_id[i], context_pos_id[i], context_segment_id[i], context_attn_bias[i], \
                    kn_id[i], labels_list[i], batch_indexs[i], batch_weight[i], batch_oneforlog[i], next_sent_context_index[i]]
        else: 
        ...
        def batch_reader(reader, batch_size):
            ...
            for instance in reader():
                if instance[3] == 1:
                    instance.append([0,1])
                    instance.append([0,1])
                    instance.append([1,0.0000000001])
                else:
                    instance.append([1,0])
                    instance.append([0,0])
                    instance.append([0.0000000001,1])
