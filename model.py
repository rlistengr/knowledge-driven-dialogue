...
class RetrievalModel(object):
    ...
    def _build_memory_network(self, kn_ids, rnn_hidden_size=128): 
        kn_emb_out = fluid.layers.embedding(
                ...
                dtype='float32')
        para_attr = fluid.ParamAttr(initializer=fluid.initializer.Normal(0.0, 0.2))
        bias_attr = fluid.ParamAttr(
                        initializer=fluid.initializer.Normal(0.0, 0.2))
    ...
    def get_pooled_output_no_share(self, context_next_sent_index, response_next_sent_index):
        ...
        context_out = fluid.layers.reduce_mean(input=self._context_reshaped_emb_out, dim=-1)
